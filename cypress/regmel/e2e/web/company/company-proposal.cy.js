describe('Cadastro de proposta', () => {
    beforeEach(() => {
        cy.visit('/login');
        cy.get('input[name="email"]').type('ceo@testcompany.com');
    ...
    it('Acessa, preenche e envia o formulário de proposta com sucesso', () => {
        ...
        cy.contains('Test Company').click();
        cy.contains('Propostas').click();
        cy.get('table > tbody > tr').first().should('contain', 'Área Teste');
        cy.get('table > tbody > tr').first().get('[data-column-id="status"').should('contain', 'Sem Adesão do Município');
    });
});