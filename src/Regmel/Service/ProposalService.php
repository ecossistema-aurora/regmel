...
readonly class ProposalService extends AbstractEntityService implements ProposalServiceInterface
{
    ...
    public function saveProposal(Organization $company, array $data, ?UploadedFile $map = null, ?UploadedFile $project = null): Initiative
    {
        ...
        if (null !== $map) {
            $mapFileName = $this->uploadFile($map, $company->getName(), $cityName, $cityCode);
        }

        if (null !== $project) {
            $projectFileName = $this->uploadFile($project, $company->getName(), $cityName, $cityCode);
        }
    ...
    private function uploadFile(UploadedFile $uploadedFile, string $company, string $municipality, string|int $cityCode, string $version = '01'): string
    {
        $pdf = $this->fileService->uploadMixedFile(
            $uploadedFile,
            extraPath: '/regmel/company/documents',
            optionalName: "Proposta-{$company}-area-poligonal-{$municipality}-{$cityCode}-{$version}",
        );
